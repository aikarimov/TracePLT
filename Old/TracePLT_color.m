...
TotalIter = 3; %общее число итераций
...
maxlenfactor = 7; % макс. длина мазка в диаметрах кисти
...
%             j = jctr;
            ...
            if (err(i,j) > pixTol) || canvas2(i,j) == 0
                ...
                [props, mixtype, ~]  = PredictProportions(rgb2hsv(double(meancol)/255), Ycell, Wcell);%определим пропорции и тип смеси в этой области
...
for i = 1:nmixgroups
    ...
    if Ncl > 0
        ...
        mixvalues = [mixvalues; mixvaluesi]; % пропорции смесей
